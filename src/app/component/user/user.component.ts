...
export class UserComponent implements OnInit, OnDestroy {
        ...
        (response: User[]) => {
          ...
          if (showNotification) {
            this.sendNotification(
              ...
              `${response.length} users(s) loaded successfully.`
            );
          }
        },
    ...
    switch (event?.type) {
      case HttpEventType.UploadProgress:
        if (event.total) {
          ...
          this.fileStatus.percentage = Math.round((100 * event.loaded) / total);
        }
  ...
  public searchUsers(searchTerm: string): void {
    const results: User[] = [];
    for (const user of this.userService.getUsersFromToLocalCache()) {
      if (
    ...
    if (results.length === 0 || !searchTerm) {
      this.users = this.userService.getUsersFromToLocalCache();
    }
  }
